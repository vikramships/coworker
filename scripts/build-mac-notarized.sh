...
# Build and notarize Agent Cowork for macOS
...
echo "🍎 Building Agent Cowork with notarization..."
...
APP_PATH="dist/mac-arm64/Agent Cowork.app"
...
cd dist/mac-arm64
zip -ry "Agent Cowork.app.zip" "Agent Cowork.app"
cd ../..

# Submit for notarization using notarytool (Apple's recommended method)
echo "📤 Submitting to Apple for notarization..."
xcrun notarytool submit \
    "dist/mac-arm64/Agent Cowork.app.zip" \
    --apple-id "$APPLE_ID" \
...
if [ $RESULT -eq 0 ]; then
    ...
    # Cleanup
    rm -f "dist/mac-arm64/Agent Cowork.app.zip"

    echo ""
    echo "🎉 Build complete with notarization!"
    echo "📦 DMG: dist/Agent Cowork-0.0.2-arm64.dmg"
    exit 0
else
