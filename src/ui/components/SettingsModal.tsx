...
export function SettingsModal({ onClose }: SettingsModalProps) {
  ...
  const populateFormFromProvider = (provider: ProviderConfig) => {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/20 px-4 py-8 backdrop-blur-sm overflow-y-auto"
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div
        className="w-full max-w-lg rounded-2xl border border-ink-900/5 bg-surface p-5 shadow-elevated my-auto max-h-[90vh] overflow-y-auto"
          ...
          <div className="mt-5 grid gap-5">
            ...
            <div className="border-t border-ink-900/10 pt-5">
              ...
              {config?.providers.map((provider) => (
                <div
                  key={provider.id}
                  className={`p-3 rounded-xl border transition-colors mb-2 cursor-pointer ${
