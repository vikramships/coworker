...
export function StartSessionModal({
  ...
  useEffect(() => {
    // Focus the textarea after the modal has rendered
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 px-4 py-8 backdrop-blur-sm overflow-y-auto animate-fade-in"
      onClick={(e) => {
    ...
    >
      <div
        className="w-full max-w-lg rounded-2xl border border-ink-900/10 bg-surface p-6 shadow-xl my-auto max-h-[90vh] overflow-y-auto animate-scale-in pointer-events-auto"
      >
        ...
        <div className="mt-5 grid gap-4">
          <label className="grid gap-1.5">
            ...
            {recentCwds.length > 0 && (
              <div className="mt-2 grid gap-2 w-full">
                <div className="text-[11px] font-medium uppercase tracking-wide text-ink-600">Recent</div>
          ...
          <button
            ...
            onClick={onStart}
            disabled={pendingStart || !cwd.trim() || !localPrompt.trim()}
          >
            {pendingStart ? (
